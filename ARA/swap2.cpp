...
bool swap2(vector<vector<int>> &G, vector<node> &rnode)//随机交换两条边，满足洋葱网络的条件则交换
{
	...
	if (alpha*(fabs(rnode[node1].degree - rnode[node11].degree) + fabs(rnode[node2].degree - rnode[node21].degree)) > (fabs(rnode[node1].degree - rnode[node21].degree) + fabs(rnode[node2].degree - rnode[node11].degree)))
	{
		G[node1][node11] = 0;
		G[node11][node1] = 0;
		G[node2][node21] = 0;
